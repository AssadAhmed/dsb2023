...
#the eprcentage pass/fail is ~the same for the train and testing data set.
```
...
from the output we can see that films that failed the bechdel test has higher mean rating (imdb/meta score) and sloghtly higher domestic/internaitonal boxoffice gross figures. The IMDB ratings for both pass/fail movies are normally distrubuted about the mean, which is similar for the meta score. The scatter plot also shows that IMDB rating and meta score are highly correlated (backed up by the ~o.7 correlation coefficient).
...
Overall the pass/fail bedechel test catagories appear to have very similar characteristics, with minor differences in the fail catagory leading to marginally higher mean box office and budget figures.
