...
#Filter out transactions without a fraud flag or amt
  filter(!is.na(is_fraud) & !is.na(amt) & is_fraud==1) %>% 
#Seperate trabsaction date into individual fields
mutate(
  date_only = lubridate::date(trans_date_trans_time),
  month_name = lubridate::month(trans_date_trans_time, label=TRUE),
  hour = lubridate::hour(trans_date_trans_time),
...
Utilise Pivot Longer and create an electricity type filed to collapse the multiple columns down to 2 (electricty type and then amount)
...
-   What, if anything, gave you the most trouble: Visual editor stopped working partway thorugh, unable to see previes of code run.
    Ran out of time to try the grsphs on energy data, initially the code would not work to populate the CO2/GDP data frames (unsure why). i ended up reopenining everything and managed to get it to work,
